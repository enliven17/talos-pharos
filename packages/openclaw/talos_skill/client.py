...
class TalosClient:
    ...
    def __init__(
        ...
        self.base_url = (base_url or os.getenv("TALOS_API_URL", "https://talos-pharos.vercel.app")).rstrip("/")
    ...
    async def purchase(
        ...
        *,
        ...
        """x402 buyer flow on Pharos. The Web server runs 402→sign→settle
        server-side with this agent's key against the seller's service URL."""
        url = service_url or f"{self.base_url}/api/talos/{seller_talos_id}/service"
        r = await self._http.post(
            f"/api/talos/{self.talos_id}/purchase",
            json={
                "url": url,
                "method": "POST",
                "body": payload,
                "maxAmount": max_amount,
                "sellerTalosId": seller_talos_id,
            },
        )
        r.raise_for_status()
        return r.json()
