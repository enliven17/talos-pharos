...
export const tlsTalos = pgTable(
  "tls_talos",
  {
    ...
    onChainId: integer("onChainId").unique(),        // Pharos TalosRegistry on-chain ID
    ...
    // Mitos Token (per-agent ERC-20 on Pharos)
    // NOTE: column name kept as `stellarAssetCode` (legacy) but now stores the
    // Mitos ERC-20 contract address (0x...). tokenSymbol holds the symbol.
    stellarAssetCode: text("stellarAssetCode"),       // Mitos ERC-20 contract address (0x...)
    ...
    // EVM addresses (0x...) — column names kept from the legacy Stellar schema
    walletPublicKey: text("walletPublicKey"),         // creator's connected EVM wallet (0x...)
    creatorPublicKey: text("creatorPublicKey"),       // creator EVM address (0x...)
    investorPublicKey: text("investorPublicKey"),
    treasuryPublicKey: text("treasuryPublicKey"),

    // Agent EVM wallet (private key held server-side / env, never in DB)
    agentWalletId: text("agentWalletId"),             // EVM address (0x...) — wallet identifier
    agentWalletAddress: text("agentWalletAddress"),   // EVM address (0x...) — payment routing
...
export const tlsCommerceServices = pgTable(
  "tls_commerce_services",
  {
    ...
    currency: text("currency").notNull().default("USDC"),
    stellarPublicKey: text("stellarPublicKey").notNull(),   // Payment recipient EVM address (0x...) — legacy column name
    chains: text("chains").array().notNull().default(["pharos"]),
...
export const tlsCommerceJobs = pgTable(
  "tls_commerce_jobs",
  {
    ...
    result: jsonb("result"),
    status: text("status").notNull().default("pending"),
    paymentSig: text("paymentSig").unique(),   // settled x402 tx hash (replay prevention)
    txHash: text("txHash"),                    // Pharos transaction hash after settlement
    amount: numeric("amount", { precision: 18, scale: 6 }).notNull(),
