...
import {
  ...
  isValidEvmAddress,
  TALOS_REGISTRY_CONTRACT,
  TALOS_NAME_SERVICE_CONTRACT,
} from "@/lib/contracts";
import { pharosAtlantic, getPublicClient, PHAROS_CHAIN_ID } from "@/lib/evm";
...
const NAME_SERVICE_WRITE_ABI = parseAbi([
  "function registerName(uint256 talosId, string name)",
]);

/** Operator/treasury EVM address (treasuryAddr). Falls back to the creator. */
const OPERATOR_ADDRESS = (process.env.NEXT_PUBLIC_OPERATOR_ADDRESS ?? "") as string;
...
export default function LaunchPage() {
  return (
    <WalletGate
      ...
      description="Creating a TALOS requires an EVM wallet on Pharos Atlantic. Your address will be registered as the Creator."
    >
      <LaunchForm />
    </WalletGate>
  );
...
function LaunchForm() {
  ...
  const handleLaunch = async () => {
    ...
    try {
      // 1. Verify EVM wallet + ensure Pharos Atlantic (688689)
      setDeployStep("Verifying EVM wallet...");
      setDeployProgress(1);
      ...
      // creatorAddr MUST equal the connected account (contract enforces this).
      const creatorAddr = address as Address;
      const treasuryAddr = (isValidEvmAddress(OPERATOR_ADDRESS)
        ? OPERATOR_ADDRESS
        : creatorAddr) as Address;
      const categoryCapitalized =
        form.category.charAt(0).toUpperCase() + form.category.slice(1);

      // 2. Register TALOS on-chain via the registry contract (if deployed)
      if (TALOS_REGISTRY_CONTRACT) {
        setDeployStep("Registering TALOS on Pharos...");
        setDeployProgress(2);

        const wallet = await getWalletClient();
        const publicClient = getPublicClient();

        // Patron shares 60/25/15 — must sum to 100.
        const patron = {
          creatorShare: 60,
          investorShare: 25,
          treasuryShare: 15,
          creatorAddr,
          investorAddr: creatorAddr,
          treasuryAddr,
        } as const;

        const kernel = {
          approvalThreshold: BigInt(Math.round(Number(form.approvalThreshold) || 10)),
          gtmBudget: BigInt(Math.round(Number(form.gtmBudget) || 100)),
          minPatronPulse: BigInt(100),
        } as const;

        const pulse = {
          totalSupply: BigInt(Number(form.totalSupply) || 1_000_000),
          priceUsdCents: BigInt(
            Math.round(parseFloat(form.initialPrice || "0.01") * 100),
          ),
        ...
        if (receipt.status !== "success") {
          throw new Error("createTalos transaction reverted");
        }

        // Parse TalosCreated to get the on-chain id.
        for (const log of receipt.logs) {
          if (log.address.toLowerCase() !== TALOS_REGISTRY_CONTRACT.toLowerCase()) {
            continue;
          }
          try {
            ...
            if (decoded.eventName === "TalosCreated") {
              onChainId = Number(decoded.args.talosId);
              break;
            }
          } catch {
            // not our event — skip
        ...
        // Contract not deployed yet — skip on-chain step
        setDeployProgress(2);
      }

      // 3. Register agent name on-chain (if name service is deployed)
      if (TALOS_NAME_SERVICE_CONTRACT && onChainId !== null) {
        ...
        const txHash = await wallet.writeContract({
          ...
          abi: NAME_SERVICE_WRITE_ABI,
          functionName: "registerName",
          args: [BigInt(onChainId), form.agentName.toLowerCase().trim()],
        });
      ...
      let dbData: { id: string; apiKeyOnce: string; agentWalletKeyOnce: string | null } | null = null;
      try {
        const res = await fetch("/api/talos", {
          ...
          body: JSON.stringify({
            ...
            // stellarAssetCode column is repurposed by the backend to hold the
            // token symbol (and later the Mitos ERC-20 address).
      ...
      setGenesisResult({
        ...
        agentWalletKey: dbData!.agentWalletKeyOnce,
        onChainId,
        agentName: form.agentName,
      });
    } catch (err) {
      ...
      let message = raw;
      if (raw.includes("User declined") || raw.includes("user rejected") || raw.includes("User rejected"))
        message = "Transaction was rejected in your wallet.";
      else if (raw.toLowerCase().includes("insufficient"))
        message = "Insufficient PHRS for gas.";
    ...
    return (
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="bg-surface border border-accent/30 p-8 mb-6 space-y-6">
          ...
          {genesisResult.agentWalletKey && (
            <div>
              <div className="text-xs text-red-400 font-bold mb-2">
                Agent Wallet Secret (shown only once)
              </div>
              <div className="bg-background border border-red-400/40 p-3 font-mono text-xs text-red-300 break-all select-all">
                {genesisResult.agentWalletKey}
              </div>
              <p className="text-xs text-muted mt-2 leading-relaxed">
                Store this as{" "}
                <span className="font-mono text-foreground">
                  TALOS_AGENT_SECRET_{genesisResult.talosId}
                </span>{" "}
                in your secret manager. This is the agent&apos;s EVM private key for
                signing x402 payments — it is never stored server-side and cannot
                be recovered.
              </p>
            </div>
          )}
      ...
      <div className="bg-surface border border-border p-8 mb-6">
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted mb-6">
              The Creator is your EVM address. It is permanently linked to this TALOS and cannot be changed.
            </p>
            <div>
              <label className="block text-xs text-muted mb-2">Creator Address (EVM)</label>
              ...
              <p className="text-xs text-muted mt-1">This EVM address will be registered as the TALOS Creator on Pharos.</p>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted mb-6">
              Configure your TALOS&apos;s Mitos token on Pharos.
            </p>
            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">2-8 characters, letters and numbers only. Used as the Mitos token symbol.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-xs text-muted mt-1">Price per Pulse token in USDC on Pharos</p>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-sm">
              ...
              <ReviewRow label="Creator (EVM)" value={form.creatorWallet || address || ""} />
