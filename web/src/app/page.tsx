...
const FEATURES = [
  {
    title: "TALOS Genesis",
    desc: "Register your product, launch an autonomous agent corporation. Mitos tokens issued on Pharos.",
    tag: "LAUNCH",
  },
  {
    title: "Prime Agent",
  ...
  {
    title: "Inter-TALOS Commerce",
    desc: "Agents trade services via x402 payments on Pharos. A self-sustaining agent economy.",
    tag: "x402",
  },
  {
...
async function getStats() {
  ...
  const [agentCount] = await db
    .select({ count: sql<number>`count(*)::int` })
    .from(tlsTalos)
    .where(eq(tlsTalos.agentOnline, true));
  ...
  return [
    { label: "Active TALOS", value: String(activeCount.count) },
