...
function isEvmAddress(addr: string | null | undefined): addr is string {
...
export function TalosDetailClient({ talos }: { talos: TalosDetail }) {
  ...
  /**
   * Send a USDC (ERC-20) payment on Pharos Atlantic and wait for the receipt.
   * Returns the tx hash. The server later records / verifies the payment.
   */
  const payUsdc = useCallback(
    async (to: string, humanAmount: string | number): Promise<string> => {
      if (!isEvmAddress(USDC_ADDRESS)) {
        throw new Error("USDC token address is not configured (NEXT_PUBLIC_USDC_ADDRESS).");
      }
      if (!isEvmAddress(to)) {
      ...
      const value = parseUnits(String(humanAmount), USDC_DECIMALS);
      ...
      if (receipt.status !== "success") {
        throw new Error("USDC payment reverted on-chain.");
      }
      return txHash;
    },
    [address, chainId, switchChain, getWalletClient],
  );
  ...
  const handleRequestService = useCallback(async () => {
    ...
    try {
      // Pay USDC on Pharos to the service recipient (agent wallet or operator).
      const recipient =
        talos.service.stellarPublicKey ||
        talos.agentWalletAddress ||
        OPERATOR_ADDRESS;

      const txHash = await payUsdc(recipient, talos.service.price);

      let payload: Record<string, unknown> = {};
      try { payload = servicePayload.trim() ? JSON.parse(servicePayload) : {}; } catch { payload = { request: servicePayload }; }

      // Server records the on-chain payment txHash and creates the job.
      ...
      alert(msg.toLowerCase().includes("insufficient") ? "Insufficient PHRS for gas." : msg);
      setServiceStatus("error");
    }
  }, [address, talos.id, talos.service, talos.agentWalletAddress, servicePayload, payUsdc]);
  ...
  const handleBuyToken = useCallback(async () => {
    ...
    setBuyStatus("buying");
    try {
      // Pay USDC on Pharos to the agent treasury (or operator fallback).
      const recipient = talos.agentWalletAddress ?? OPERATOR_ADDRESS;
      const txHash = await payUsdc(recipient, buyCost.toFixed(USDC_DECIMALS));

      // Record the purchase — server sends Mitos (ERC-20) to the buyer.
  ...
  }, [address, buyQty, buyCost, talos.id, talos.agentWalletAddress, payUsdc]);
            ...
            <div className="bg-surface border border-border p-6">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-foreground">Pharos Atlantic</span>
                </div>
                <div className="flex justify-between gap-2">
                  <span className="text-muted shrink-0">Mitos Token</span>
                  <span className="text-foreground font-mono truncate text-right">
                    {isEvmAddress(talos.stellarAssetCode)
                ...
                {talos.agentWalletAddress && (
                  <a
                    href={`${EXPLORER_URL}/address/${talos.agentWalletAddress}`}
                    target="_blank"
            ...
            <>
              ...
              <div className="bg-surface border border-border p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-foreground mt-1">x402 on Pharos</p>
                  </div>
                  <div>
              ...
              <div className="bg-surface border border-border p-6">
                ...
                <div className="bg-background border border-border p-4 text-xs text-foreground overflow-x-auto font-mono space-y-1">
                  <div className="text-green-400"># 1. Send USDC payment on Pharos</div>
                  ...
                  <div className="pl-4">&quot;txHash&quot;: &quot;&lt;pharos_tx_hash&gt;&quot;,</div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface border border-border p-6">
            ...
            <div className="bg-background border border-border p-4 text-xs text-foreground space-y-1 overflow-x-auto font-mono">
              ...
              <div className="text-muted">Network:   Pharos Atlantic</div>
            ...
            {buyStatus === "success" && buyResult ? (
              ...
              <div className="px-6 py-6 space-y-5">
                ...
                <p className="text-xs text-muted/50 text-center">
                  Pharos Atlantic testnet &mdash; real transaction, real tokens
                </p>
              </div>
            )}
          </div>
              ...
              <div className="px-6 py-6 space-y-5">
                ...
                <div className="bg-surface border border-border p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="text-foreground">Pharos · wallet signing</span>
                  </div>
                </div>
