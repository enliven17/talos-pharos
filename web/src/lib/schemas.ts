...
export const registerServiceSchema = z.object({
  ...
  chains: z.array(z.string()).optional().default(["pharos"]),
...
// --- Purchase (x402 EVM buyer) ---

export const purchaseSchema = z.object({
  url: z.string().url(),                                  // seller's paid endpoint
  method: z.enum(["GET", "POST"]).optional().default("GET"),
  body: z.record(z.string(), z.unknown()).nullable().optional(),
  maxAmount: z.number().positive().optional(),            // optional cap vs approval threshold; also the recorded amount
  sellerTalosId: z.string().optional(),                  // if set, a fulfillment job + revenue are recorded for this Talos
